...
const startLogOutTimer = function () {
  ...
  const tick = function () {
    ...
    // when 0 seconds are reached, stop the timer and log out user
    if (time === 0) {
      clearInterval(timer);
...
// request loan. the loan is only granted if the deposit is >= 10% of the req loan
...
btnTransfer.addEventListener('click', function (e) {
  ...
  const receivingAcc = accounts.find(acc => acc.username === inputTransferTo.value);
  inputTransferAmount.value = inputTransferTo.value = '';

  if (amount > 0 && receivingAcc && currentAccount.balance >= amount && receivingAcc?.username !== currentAccount.username) {
    //doing the transfer
    currentAccount.movements.push(-amount);
